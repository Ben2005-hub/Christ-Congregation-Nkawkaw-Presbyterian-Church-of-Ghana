...
async function sendSms(to, text, options = {}) {
  ...
  try {
    ...
    if (process.env.SMS_PROVIDER === 'akasel' || process.env.SMS_PROVIDER === 'arkesel') {
      // Arkesel (Akasel) public API uses query params. Example:
      // https://sms.arkesel.com/sms/api?action=send-sms&api_key=KEY&to=Phone&from=Sender&sms=Message
...
// Additional helpers for Arkesel
