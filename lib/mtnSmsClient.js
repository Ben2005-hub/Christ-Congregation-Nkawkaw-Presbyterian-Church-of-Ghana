...
const SMS_PROVIDER = process.env.SMS_PROVIDER || process.env.MTN_ENV || 'mtmtn';
// MTN-specific
...
const SENDER = process.env.MTN_SMS_SENDER || process.env.SMS_SENDER;
const ENV = process.env.MTN_ENV || 'sandbox';
const SIMULATE = (process.env.MTN_ENV === 'simulate') || (process.env.SMS_PROVIDER === 'simulate');
...
async function sendSms(to, text, options = {}) {
  ...
  try {
    ...
    // Akasel provider (simple API key + JSON body)
    if (process.env.SMS_PROVIDER === 'akasel') {
      if (!AKASEL_URL || !AKASEL_KEY) throw new Error('Akasel API not configured (AKASEL_API_URL / AKASEL_API_KEY)');
      const url = AKASEL_URL.replace(/\/$/, '') + '/send';
      const payload = {
      ...
      const headers = {
        ...
        'x-api-key': AKASEL_KEY
      };
      const resp = await axios.post(url, payload, { headers, timeout: 10000 });
      logEntry.status = 'sent';
      logEntry.messageId = resp.data.id || resp.data.messageId || resp.data.reference;
      await db.logMessage(logEntry);
      return { success: true, messageId: logEntry.messageId, response: resp.data };
    }
    ...
    const url = `${SMS_BASE.replace(/\/$/, '')}/v3/sms/messages/sms/outbound`;
